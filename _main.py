...
def processData(v):
	...
	if len(times) == 0:
		...
		frecv = len(times) / (time.time() - times[0])
...
def startRound():
	...
	target_label = Label(C2, text = "Target (120 bpm)", font=("Courier", 40))
	target_label.place(x=10, y=20)

	you_label = Label(C2, text = "You (0 bps)", font=("Courier", 40))
	you_label.place(x=10, y=320)

	c_plot1 = Canvas(C2, bg="black", height=200, width=580)
	c_plot1.place(x=10, y=90)

	c_plot2 = Canvas(C2, bg="black", height=200, width=580)
